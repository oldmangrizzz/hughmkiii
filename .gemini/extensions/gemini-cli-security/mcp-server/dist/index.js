...
server.tool('find_line_numbers', 'Finds the line numbers of a code snippet in a file.', {
    filePath: z
        .string()
...
server.tool('get_audit_scope', 'Checks if the current directory is a GitHub repository.', {}, () => {
...
server.tool('run_poc', 'Runs the generated PoC code.', {
    filePath: z.string().describe('The absolute path to the PoC file to run.'),
}, (input) => runPoc(input));
server.registerPrompt('security:note-adder', {
    title: 'Note Adder',
...
server.registerPrompt('security:poc', {
    title: 'PoC Generator',
...
server.registerPrompt('security:scan_deps', {
