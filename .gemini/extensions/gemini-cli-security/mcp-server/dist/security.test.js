...
describe('findLineNumbers', () => {
    ...
    const mockFs = {
        ...
        readFile: (p, options) => Promise.resolve(''),
    };
    ...
    `;
        ...
        const result = await findLineNumbers({ filePath: mockFilePath, snippet: 'const b = 2;' }, { fs: testFs, path });
    ...
    `;
        ...
        const result = await findLineNumbers({ filePath: mockFilePath, snippet: `
        console.log('hello');
        console.log('world');
      ` }, { fs: testFs, path });
        const parsedResult = JSON.parse(result.content[0].text);
    ...
    `;
        ...
        const result = await findLineNumbers({ filePath: mockFilePath, snippet: 'const z = 30;' }, { fs: testFs, path });
        const parsedResult = JSON.parse(result.content[0].text);
        expect(parsedResult.error).toBe('Snippet was not found.');
    });
    it('should handle an empty snippet', async () => {
        const result = await findLineNumbers({ filePath: 'mock.ts', snippet: '' }, { fs: mockFs, path });
        const parsedResult = JSON.parse(result.content[0].text);
    ...
    it('should handle file not found error', async () => {
        ...
        const result = await findLineNumbers({ filePath: mockFilePath, snippet: 'any' }, { fs: testFs, path });
        const parsedResult = JSON.parse(result.content[0].text);
